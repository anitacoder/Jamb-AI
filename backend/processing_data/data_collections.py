...
class JambDataCollector:
    ...
    def __init__(self, db_instance, base_data_dir="data-preparation"):
        ...
        self.jamb_urls = [
            "https://myschool.ng/jamb/past-questions",
...
if __name__ == "__main__":
    ...
    os.environ['MONGO_DB_NAME'] = os.getenv('MONGO_DB_NAME', "jamb_db")

    jamb_db_instance = setup_jamb_db()

    if jamb_db_instance:
        logger.info("Starting JAMB data collection...")
        collector = JambDataCollector(jamb_db_instance)

        web_docs_collected = collector.collect_web_data()
        logger.info(f"Total web documents collected/updated: {web_docs_collected}")

        pdf_docs_collected = collector.collect_pdf_data()
        logger.info(f"Total PDF documents collected/updated: {pdf_docs_collected}")

        collector.organize_by_year()
        logger.info("Data organization complete.")

        jamb_db_instance.close_connection()
    else:
        logger.error("Failed to initialize JAMB database. Cannot proceed with data collection.")